...
import { onEventosChange, adicionarEvento, alternarEvento } from "../services/firestore";
...
const s = {
  ...
  cell: (isToday, otherMonth, hasTask) => ({
    ...
    paddingTop:4, borderRadius:6, fontSize:12,
  ...
  taskItem: (removing) => ({
    ...
    opacity: removing ? 0 : 1, transform: removing ? "scale(0.95)" : "scale(1)",
    overflow:"hidden", maxHeight: removing ? 0 : 80,
  }),
...
function CheckCircle({ checked, onClick }) {
  return (
    <button onClick={onClick} style={s.checkCircle(checked)} aria-label="Marcar como concluída" role="checkbox" aria-checked={checked}>
...
function TaskItem({ task, onToggle }) {
  ...
  const handleToggle = () => { setRemoving(true); setTimeout(() => onToggle(task.id), 350); };
...
function Calendar({ tasks, year, month, onPrev, onNext }) {
  ...
  for (let i = 1; i <= rem; i++) cells.push({ day: i, otherMonth: true, tasks: [] });
  ...
  return (
    <div style={s.panel}>
      ...
      <div style={s.calGrid}>
        ...
        {cells.map((cell, i) => {
          const isToday = !cell.otherMonth && cell.dateStr === todayStr;
          return (
...
function Checklist({ tasks, onToggle, onAdd, carregando }) {
  ...
  return (
    <div style={s.panel}>
      <div style={s.panelTitle}>Tarefas do Calendário</div>
      ...
      <div style={s.addRow}>
        <input
          style={s.addInput} placeholder="Nova tarefa..." value={name} maxLength={60}
...
export default function CalendarChecklist() {
  ...
  // Escuta eventos do Firestore em tempo real
  useEffect(() => {
    const unsubscribe = onEventosChange((dados) => {
      setTasks(dados);
      setCarregando(false);
    });
    return () => unsubscribe();
  ...
  const handleToggle = useCallback(async (id) => {
    await alternarEvento(id);
  }, []);

  const handleAdd = useCallback(async (name, date) => {
    const color = COLORS[colorIdx % COLORS.length];
    colorIdx++;
    await adicionarEvento({ name, date, color });
  }, []);
      ...
      <main className="conteudo-planner">
        <div className="planner-container-restrito">
          ...
          <div className="col-calendario">
            <Checklist tasks={tasks} onToggle={handleToggle} onAdd={handleAdd} carregando={carregando} />
          </div>
        </div>
      </main>
    </div>
